...
/**
 * 遮罩系统的封装 <br>
 * 外部需要调用{@link  com.lst.burns.guideview.GuideBuilder}来创建该实例，实例创建后调用
...
public class Guide implements View.OnKeyListener, View.OnClickListener {
    ...
    /**
     * 显示该遮罩, <br>
     * 外部借助{@link com.lst.burns.guideview.GuideBuilder}
    ...
    private MaskView onCreateView(Activity activity) {
        ...
        if (mConfiguration.mTargetView != null) {
            Log.d("ZFH","parentX " + parentX + " parentY " + parentY );
        ...
        if (fulling != null) {
            Log.d("ZFH","fulling is not null");
            maskView.setFullingRect(Common.getViewAbsRect(fulling, parentX, parentY));
        }
        Log.d("ZFH","mOutsideTouchable " + mConfiguration.mOutsideTouchable);
