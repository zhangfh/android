...
public class DrawView extends View {
    ...
    * that were specified in the XML file.*/
    public DrawView(Context context, AttributeSet attr) {
        super(context, attr);
        Log.d("ZFH","DrawView 2");
        init();
    }
    /** Simple constructor to use when creating a view from code.*/
    public DrawView(Context context) {
        super(context);
        Log.d("ZFH","DrawView 3");
        init();
    }
    ...
    private void init() {
        ...
        Log.d("ZFH","mImageWidth " + mImageWidth + " mImageHeight " + mImageHeight);
        ...
        mPaint.setStrokeJoin(Paint.Join.ROUND);//设置结合处的样式
        mPaint.setStrokeCap(Paint.Cap.ROUND);//设置画笔的cap属性,设置笔刷的图形样式，如圆形样式
        mPaint.setStrokeWidth(12);//设置笔刷的粗细度
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        Log.d("ZFH","onSizeChanged w " + w + " h: " + h + " oldw " + oldw + " oldh " + oldh);
        mImageWidth = w;
        mImageHeight = h;
    ...
    private Map<String, Float> getImageViewIneerSize() {
        ...
        //Log.d("ZFH","matrix " + matrix);
        matrix.getValues(values);
        //Log.d("ZFH","values " + values);
        //Image在绘制过程中的变换矩阵，从中获得x和y方向的缩放系数
        float sx = values[0];//MSCALE_X
        float sy = values[4];//MSCALE_Y
        //计算Image在屏幕上实际绘制的宽高
        size.put("scaleX", 1 / sx);
        size.put("scaleY", 1 / sy);
        size.put("offsetX", values[2]); //X轴的translate的值 MTRANS_X
        size.put("offsetY", values[5]);//Y轴的translate的值 MTRANS_Y trans是平移
        return size;
    }
    ...
    public boolean setBitmap(Bitmap imagePath) {
        ...
        if (width != 0 && height != 0) {//如果图片的宽高不等于0代表图片显示成功
            ...
            Log.d("ZFH","nxScale "+ nxScale + " nyScale " + nyScale);
            ...
            //Log.d("ZFH","matix " + matrix);
            matrix1.set(matrix);
            //Log.d("ZFH","matrix1 " + matrix1);
            saveMatrix.set(matrix);
            //Log.d("ZFH","saveMatrix " + saveMatrix);
            getImageViewIneerSize();
    ...
    public String saveImage() {
        ...
        Log.d("ZFH","path: " + Environment.getExternalStorageDirectory().getPath());
        //创建存放缓存的文件夹
        ...
        try {
            ...
            mBottomBitmap.compress(Bitmap.CompressFormat.PNG, 80, out);
    ...
    public String savePhotoImage(String graphPath) {
        ...
        try {
            ...
            FileOutputStream out = new FileOutputStream(file);
            mBottomBitmap.compress(Bitmap.CompressFormat.PNG, 100, out);
            mBottomBitmap.recycle();
            mBitmap.recycle();
            out.flush();
            out.close();
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        //Log.d("ZFH","map1 " + map1);
       // Log.d("ZFH","onTouchEvent x:"+ x + " y:" + y + " pointCount:" + pointCount);
        switch (event.getAction() & MotionEvent.ACTION_MASK) {
            case MotionEvent.ACTION_DOWN://把画笔移动到触摸处开始绘画
                mode = DRAW;
                //Log.d("ZFH", "ACTION_DOWN");
                mPath = new Path();
                mCaPath = new Path();
