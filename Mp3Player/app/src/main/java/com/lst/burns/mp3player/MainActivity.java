...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mActions.setOnTouchListener(new View.OnTouchListener() {

            @Override
            public boolean onTouch(View view, MotionEvent event) {
                if (event.getAction() == MotionEvent.ACTION_DOWN){
                    origin[0] = (int) event.getX();
                    Log.d("ZFH","ACTION_DOWN x " +origin[0]);
                } else if (event.getAction() == MotionEvent.ACTION_UP) {
                    //int distance = ((int)event.getX()) - origin[0];


                    if (origin[0] > PlayPauseButton.getX() + PlayPauseButton.getWidth())
                        mService.playNext();
                    else if (origin[0] < PlayPauseButton.getX() - PlayPauseButton.getWidth())
                        mService.playLast();
        ...
        mMusicTitle = (TextView)findViewById(R.id.current_title);
    ...
    private void defineServiceConnection() {
        mConnection = new ServiceConnection() {
            ...
            public void onServiceConnected(ComponentName name, IBinder service) {
                ...
                mService.registerTextView(mMusicTitle);
                mBound = true;
    ...
    private void createLibrary(final FileFlattener ff) {
        new AsyncTask<Void, Void, Void>() {
            ...
            protected void onPostExecute(Void params) {
                ...
                Log.d("ZFH", "music have " + filePaths.size());
                mMusicViewAdapter.notifyDataSetChanged();
